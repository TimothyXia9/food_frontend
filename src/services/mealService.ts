...
class MealService {
	...
	async getUserMeals(params: MealListParams = {}): Promise<ApiResponse<{ meals: Meal[]; total_count: number; page: number; page_size: number; total_pages: number }>> {
		...
		return apiClient.get<{ meals: Meal[]; total_count: number; page: number; page_size: number; total_pages: number }>("/meals/list/", processedParams as unknown as Record<string, unknown>);
	}
	...
	async addFoodToMeal(mealId: number, data: AddFoodToMealRequest): Promise<ApiResponse<{ meal_food_id: number; food: object; quantity: number; calories: number }>> {
		return apiClient.post<{ meal_food_id: number; food: object; quantity: number; calories: number }>(`/meals/${mealId}/add-food/`, data);
	}

	async updateMealFood(mealFoodId: number, data: UpdateFoodInMealRequest): Promise<ApiResponse<{ meal_food_id: number; new_quantity: number; new_calories: number; message: string }>> {
		return apiClient.put<{ meal_food_id: number; new_quantity: number; new_calories: number; message: string }>(`/meals/food/${mealFoodId}/update/`, data);
	}
	...
	async createMealPlan(data: { start_date: string; end_date: string; meal_template: MealTemplate }): Promise<ApiResponse<{ meals_created: number; start_date: string; end_date: string }>> {
	...
	async getMealStatistics(date: string, mealType?: string): Promise<ApiResponse<object>> {
		...
		return apiClient.get<object>("/meals/statistics/", params as unknown as Record<string, unknown>);
	}

	async getMealComparison(date1: string, date2: string, mealType?: string): Promise<ApiResponse<object>> {
		const params: MealComparisonParams = { date1, date2 };
		if (mealType) {
			params.meal_type = mealType;
		}
		return apiClient.get<object>("/meals/comparison/", params as unknown as Record<string, unknown>);
	}
